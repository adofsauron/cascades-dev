...
#define LOG2HTBL 13               // LOG2 of number of idx_hash buckets to hold mexprs.
#define HtblSize (1 << LOG2HTBL)  // idx_hash table size we used
...
typedef enum ORDER
{
  ...
  heap,
  sorted,
  hashed  // assume unique idx_hash function for entire system
} ORDER;
...
{
  idx_btree,
  idx_hash  // assume unique idx_hash function for entire system
} ORDER_INDEX;
