/*
group.cpp -  implementation of class M_GROUP
        $Revision: 12 $
        Implements class M_GROUP as defined in ssp.h
  ...
  */
  ...
  if (COVETrace)  // New Group
  {
    ...
    if (arity)
    {
      for (int i = 0; i < arity; i++)
      {
        (void)_itoa(MExpr->GetInput(i), buffer, 10);
        temp = temp + " " + buffer;
      }
    }
    temp += " ";
    ...
    os.Format("addGroup { %d %d \" %s  \"%s} %s",
              ...
              GroupID, int(MExpr), MExpr->GetOp()->Dump(), temp, LogProp->DumpCOVE());
...
#ifndef IRPROP
  for (int i = 0; i < Winners.GetSize(); i++) delete Winners[i];
#endif
}

// estimate the number of tables in EQJOIN
// ##ModelId=3B0C086701FC
int M_GROUP::EstimateNumTables(M_EXPR *MExpr)
{
  int table_num;
  int total = 0;
  M_GROUP *Group;
...
void M_GROUP::NewMExpr(M_EXPR *MExpr)
{
  // link to last mexpr
  if (MExpr->GetOp()->is_logical())
  {
...
void M_GROUP::set_optimized(bool is_optimized)
{
...
void M_GROUP::DeletePhysMExpr(M_EXPR *PhysMExpr)
{
