...
CAT::CAT(CString filename)
{
  ...
  COLL_PROP *CollProp = nullptr;
  ATTR *Attribute = nullptr;
  IND_PROP *Index = nullptr;
  BIT_IND_PROP *BitIndexProp = nullptr;
...
CAT::~CAT()
{
  ...
  for (i = 1; i < CollProps.GetSize(); i++) delete CollProps[i];

  // free attributes
  for (i = 1; i < Attrs.GetSize(); i++) delete Attrs[i];

  // free index properties
  for (i = 1; i < IndProps.GetSize(); i++) delete IndProps[i];

  // free bit index properties
  for (i = 1; i < BitIndProps.GetSize(); i++) delete BitIndProps[i];

  // free attribute names table
  for (i = 1; i < AttNames.GetSize(); i++) delete AttNames[i];

  // free index names table
  for (i = 1; i < IndNames.GetSize(); i++) delete IndNames[i];

  // free bitindex names table
  for (i = 1; i < BitIndNames.GetSize(); i++) delete BitIndNames[i];
  CollTable.RemoveAll();
  AttTable.RemoveAll();
  IndTable.RemoveAll();
