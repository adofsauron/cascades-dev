...
PHYS_PROP *FILE_SCAN::FindPhysProp(PHYS_PROP **input_phys_props)
{
  ...
  else
  {
    ...
    if (CollProp->Order == sorted)
      for (int i = 0; i < CollProp->KeyOrder.GetSize(); i++) phys_prop->KeyOrder.Add(CollProp->KeyOrder[i]);
    return phys_prop;
  }
...
HASH join
=========
       Like Merge join, but inputs can have any property.  Operator constructs a idx_hash table.
*/
...
COST *HASH_JOIN::FindLocalCost(LOG_PROP *LocalLogProp, LOG_PROP **InputLogProp)
{
  ...
  COST *result = new COST(RightCard * Cm->hash_cost()      // cpu cost of building idx_hash table
                          + LeftCard * Cm->hash_probe()    // cpu cost of finding idx_hash bucket
                          + OutputCard * Cm->touch_copy()  // cpu cost of copying result
  );                                                       // no i/o cost
...
COST *HASH_DUPLICATES::FindLocalCost(LOG_PROP *LocalLogProp, LOG_PROP **InputLogProp)
{
  ...
  COST *result = new COST(InputCard * Cm->hash_cost()  // cpu cost of hashing
                                                       // assume idx_hash collisions add negligible cost
                          + OutputCard * Cm->touch_copy()  // cpu cost of copying result
                                                           // no i/o cost
  );
...
CString HGROUP_LIST::Dump()
{
  ...
  if (GbySize > 0)
    temp.Format("%s )", GetAttName(GbyAtts[i]));
  else
...
COST *BIT_JOIN::FindLocalCost(LOG_PROP *LocalLogProp, LOG_PROP **InputLogProp)
{
  ...
  COST *result = new COST(LeftCard * Cm->cpu_read()    // cpu cost of reading bit vector
                          + LeftCard * Cm->cpu_pred()  // cpu cost of check bit vector
                                                       ...
                                                       //		+OutputCard									// number of result
                                                       //tuples
                                                       //		 * Cm->touch_copy()							// cpu cost of projecting
                                                       //and
                                                       // copying result
...
COST *INDEXED_FILTER::FindLocalCost(LOG_PROP *LocalLogProp, LOG_PROP **InputLogProp)
{
  ...
  for (int i = 0; i < Indices->GetSize(); i++)
  {
    ...
    if (IndexId == FreeVar[0]                              // FreeVar is contained in index list
        && Cat->GetIndProp(IndexId)->IndType == idx_btree  // index type is idx_btree
