...
class COLL_PROP
{
 public:
  ...
  COLL_PROP &operator=(COLL_PROP &other)  //  = operator
  {
    ...
    Order = other.Order;
    if (Order == sorted)
      for (int i = 0; i < other.KeyOrder.GetSize(); i++) KeyOrder.Add(other.KeyOrder[i]);
    return *this;
  };
  ...
  ~COLL_PROP()
  {
    delete Keys;
    delete CandidateKey;
    for (int i = 0; i < FKeyArray.GetSize(); i++) delete FKeyArray[i];
  };
...
class IND_PROP
{
 public:
  ...
  ORDER_INDEX IndType;  // idx_hash or sort (Btree)
...
class LOG_COLL_PROP : public LOG_PROP
{
  ...
  */
  ...
  ~LOG_COLL_PROP()
  {
    delete Schema;
    delete CandidateKey;
    for (int i = 0; i < FKeyList.GetSize(); i++) delete FKeyList[i];
    if (TraceOn && !ForGlobalEpsPruning)
