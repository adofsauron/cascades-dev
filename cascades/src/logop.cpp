...
GET::GET(CString collection, CString rangeVar)
{
  ...
  else
  {
    ...
    for (int i = 0; i < Size; i++)  // For each attribute
    {
      ...
      Cat->AddAttr(RangeVar, TruncName(GetAttName(AttArray->GetAt(i))), attr, domain);
    }
    ...
    if (IndArray)
    {
      ...
      for (int i = 0; i < Size; i++)  // For each index
      {
        ...
        *indprop = *ip;
        ...
        Cat->AddIndex(RangeVar, TruncName(GetIndName(IndArray->GetAt(i))), indprop);
      }
    }
    ...
    if (BitIndArray)
    {
      ...
      for (int i = 0; i < Size; i++)  // For each index
      {
        ...
        *bitindprop = *ip;
        ...
        Cat->AddBitIndex(RangeVar, TruncName(GetBitIndName(BitIndArray->GetAt(i))), bitindprop);
      }
    }
...
EQJOIN::EQJOIN(EQJOIN &Op) : lattrs(CopyArray(Op.lattrs, Op.size)), rattrs(CopyArray(Op.rattrs, Op.size)), size(Op.size)
{
  if (TraceOn && !ForGlobalEpsPruning)
    ClassStat[C_EQJOIN].New();
#ifdef _DEBUG
