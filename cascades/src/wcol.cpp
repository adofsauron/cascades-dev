...
BOOL CWcolApp::InitInstance()
{
  ...
  pDocTemplate = new CSingleDocTemplate(IDR_MAINFRAME, RUNTIME_CLASS(CWcolDoc),
  ...
  ifstream fin(OptionFile.GetBuffer(OptionFile.GetLength()));
...
void CWcolView::Print(LPCTSTR lpszMessage) { GetEditCtrl().ReplaceSel(lpszMessage); }
...
#define MAX_MEM 0xC0000000
  ...
  while ((LONG)lpScanAt < MAX_MEM)
  {
    ...
    if (mbi.AllocationBase != lpRegionBase)
    {
      ...
      {
        out_string.Format("Module: %s\r\nBaseAddress: %#x Size: %d\r\nMemoryUsed: %d", Trim(szModuleName), lpRegionBase,
                          mbi.RegionSize, ms.dwTotalVirtual - ms.dwAvailVirtual);
        break;
      }
    }
...
COptionDialog::COptionDialog(CWnd *pParent /*=NULL*/) : CDialog(COptionDialog::IDD, pParent)
{
...
BOOL COptionDialog::OnInitDialog()
{
  ...
  if (!m_pToolTip)
  {
    ...
    rt = m_pToolTip->AddTool(GetDlgItem(IDC_COVE),
                             "Write input for COVE (Columbia Optimizer Visualizer Environment) to cove.txt");
    ASSERT(rc != 0);
