...
ub4 lookup2(register ub4 k,        // the key to be hashed
            register ub4 initval)  // the previous idx_hash, or an arbitrary value
{
  register ub4 a, b, c;

  /* Set up the internal state */
  a = b = 0x9e3779b9; /* the golden ratio; an arbitrary value */
  c = initval;        /* the previous idx_hash value */
...
void IND_PROP::update(CString NewName) { Keys->update(NewName); }
...
void BIT_IND_PROP::update(CString NewName) { BitAttr->update(NewName); }
    ...
    for (index = 0; index < this->Size; index++)
    {
      if (attrs[i] == this->Attrs[index]->AttId)
      {
        ...
        for (int i = 0; i < new_schema->TableNum; i++)
          if (CollId == new_schema->TableId[i])
            break;
...
{
  if (strcmp(p, "idx_btree") == 0)
    return idx_btree;
  if (strcmp(p, "idx_hash") == 0)
    return idx_hash;
    ...
    : Keys(other.Order == any ? NULL : new KEYS_SET(*(other.Keys))), Order(other.Order)
{
  ...
  if (Order == sorted)
  {
    ...
    for (int i = 0; i < other.KeyOrder.GetSize(); i++) this->KeyOrder.Add(other.KeyOrder[i]);
  }
}
