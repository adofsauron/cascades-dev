...
by logical equivalence. We will call each partition a M_GROUP.  Thus a M_GROUP
...
Each M_GROUP corresponds to a temporary collection, or a subquery, in a 
...
public:
	...
	inline M_GROUP *	GetGroup(GRP_ID Gid) { return Groups[Gid]; } ;	
	...
	//GRP_ID MergeGroups(M_GROUP & ToGroup, M_GROUP & FromGroup);
...
private:
	...
	M_EXPR * HashPtr;			// list within idx_hash bucket
...
a search to find the cheapest MultiExpression in a M_GROUP, 
satisfying some context. 
   
A M_GROUP includes a collection of logically equivalent M_EXPR's.
The M_GROUP also contains logical properties shared by all M_EXPR's 
in the group.
	 
A M_GROUP also contains a winner's circle consisting of winners from 
previous searches of the M_GROUP.  Note that each search may of the
M_GROUP may have different contexts and thus different winners.
	   
A M_GROUP can also be thought of as a temporary collection, or
...
public:
	...
	M_GROUP(M_EXPR * MExpr); //Create a new Group containing just this MExpression
...
private	: 
	...
	M_EXPR * FirstLogMExpr;	//first log M_EXPR in  the M_GROUP
	//##ModelId=3B0C08670116
	M_EXPR * LastLogMExpr;	//last log M_EXPR in  the M_GROUP
	//##ModelId=3B0C0867012B
	M_EXPR * FirstPhysMExpr;	//first phys M_EXPR in  the M_GROUP
	//##ModelId=3B0C08670149
	M_EXPR * LastPhysMExpr;	//last phys M_EXPR in  the M_GROUP
	
	//##ModelId=3B0C08670167
	struct BIT_STATE State;		//  the state of the group
	
	//##ModelId=3B0C08670185
	LOG_PROP *	 LogProp;       //Logical properties of this M_GROUP
   ...
   it a winner's circle (cf. the M_GROUP class).  
