...
SSP::~SSP()
{
  for (int i = 0; i < Groups.GetSize(); i++) delete Groups[i];
  for (int j = 0; j < M_WINNER::mc.GetSize(); j++) delete M_WINNER::mc[j];
...
void SSP::ShrinkGroup(GRP_ID group_no)
{
  ...
  while (mexpr != NULL)
  {
    // maintain the idx_hash link
    // find my self in the appropriate idx_hash bucket
    ub4 hashval = mexpr->idx_hash();
    for (p = HashTbl[hashval], prev = NULL; p != mexpr; prev = p, p = p->GetNextHash())
      ;

    assert(p == mexpr);
    // link prev's next idx_hash to next
    if (prev)
...
M_EXPR *SSP::FindDup(M_EXPR &MExpr)
{
  ...
  if (!ForGlobalEpsPruning)
    OptStat->HashedMExpr++;
  // try all expressions in the appropriate idx_hash bucket
  ...
  not_a_duplicate:
    continue;  // check next expression in idx_hash bucket
  }            // try all expressions in the appropriate idx_hash bucket
...
void SSP::CopyOut(GRP_ID GrpID, PHYS_PROP *PhysProp, int tabs)
{
  ...
  M_GROUP *ThisGroup = Ssp->GetGroup(GrpID);
...
bool M_GROUP::firstplan = false;
#endif

/* bool M_GROUP::search_circle(CONT * C, bool & moresearch)
    {
...
bool M_GROUP::search_circle(int GrpNo, PHYS_PROP *PhysProp, bool &moreSearch)
{
...
WINNER *M_GROUP::GetWinner(PHYS_PROP *PhysProp)
{
...
void M_GROUP::NewWinner(PHYS_PROP *ReqdProp, M_EXPR *MExpr, COST *TotalCost, bool done)
{
  if (COVETrace && MExpr)  // New Winner
  {
    CString os;
    os.Format("NewWin %d \"%s\"%s  { %d %d \"%s\" %s }\r\n", MExpr->GetGrpID(), ReqdProp->Dump(), TotalCost->Dump(),
              MExpr->GetGrpID(), int(MExpr), MExpr->Dump(), done ? "Done" : "Not Done");
...
void SSP::optimize()
{
  ...
  {
    CString os;
    os.Format("%d\t%d\t%d\t%d\t%d\t", TaskNo, ClassStat[C_GROUP].Count, ClassStat[C_M_EXPR].Count,
              ClassStat[C_M_EXPR].Total, OptStat->FiredRule);
    OUTPUT("%s", os);
  }
  else
  {
