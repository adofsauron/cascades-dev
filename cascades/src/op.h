...
// Arguments could be attributes to project on, etc.
{
 public:
  ...
  // Used to compute the idx_hash value of an mexpr.  Used only for logical operators,
  // so we make it abort everywhere else.
  // ##ModelId=3B0C08720126
  virtual ub4 idx_hash() = 0;
...
class AGG_OP
{
 ...
 public:
  ...
  AGG_OP(CString range_var, int *atts, int size) : RangeVar(range_var), Atts(atts), AttsSize(size)
  {
    if (TraceOn && !ForGlobalEpsPruning)
  ...
  CString Dump()
  {
    ...
    if (AttsSize == 0)
      ...
      temp.Format("%s", GetAttName(Atts[AttsSize - 1]));
    os += temp;
