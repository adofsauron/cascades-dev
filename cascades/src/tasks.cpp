...
O_GROUP::perform
{
    see search_circle in declaration of class M_GROUP, for notation
...
void O_GROUP::perform()
{
  ...
  M_GROUP *Group = Ssp->GetGroup(GrpID);
...
void O_INPUTS::perform()
{
  ...
  M_GROUP *LocalGroup = Ssp->GetGroup(MExpr->GetGrpID());  // Group of the MExpr
  ...
  if (InputNo == -1)
  {
    ...
    {
      M_GROUP *InputGroup = Ssp->GetGroup(MExpr->GetInput(input));  // Group of current input
    ...
    for (input = 0; input < arity; input++)
    {
      ...
      PTRACE2("search_circle(): more search %s needed, return value is %s", moreSearch ? "" : "not",
              SCReturn ? "true" : "false");
  ...
  {
    PTRACE2("Expr LowerBd %s, exceed Cond UpperBd %s,Pruning applied!", CostSoFar.Dump(), LocalUB->Dump());
  ...
  for (input = InputNo; input < arity; input++)
  {
    ...
    if (Op->is_physical())
    {
      ...
      if (Pruning)
        assert(possible);  // should be possible since in the first pass, we checked it
      if (!possible)
      {
        delete ReqProp;
        goto TerminateThisTask;
  ...
  if (arity == 0 && LocalReqdProp->GetOrder() != any && Op->is_physical())
  {
    ...
    {
      PTRACE2("physical epxr: %s does not satisfy required phys_prop: %s", MExpr->Dump(), LocalReqdProp->Dump());
  ...
  if (GlobepsPruning)
  {
    ...
    if (*EpsBound >= CostSoFar)
    {
      ...
      PTRACE("Global Epsilon Pruning fired, %s", "got a final winner for this context");
  ...
  if (Halt)
  {
    if (LocalGroup->GetFirstLogMExpr()->GetOp()->GetName().Compare("EQJOIN") == 0)
    {
      ...
      {
        PTRACE("Halting condition satisfied, %s", "got a final winner for this context");
  ...
  {
    PTRACE2("total cost too expensive: totalcost %s >= upperbd %s", CostSoFar.Dump(), LocalUB->Dump());
  ...
  {
    M_GROUP *Group = Ssp->GetGroup(GrpNo);
    COST *WinCost = new COST(CostSoFar);
...
TerminateThisTask:
  ...
  if (Last)
  {
    ...
    os.Format("Terminate: replaced winner with %s, %s, %s\r\n", LocalReqdProp->Dump(),
              TempME ? TempME->Dump() : " NULL ", LocalWinner->GetCost()->Dump());
    PTRACE("%s", os);
#endif
  }
#endif
...
APPLY_RULE::~APPLY_RULE()
{
  if (Last)
  {
    M_GROUP *Group = Ssp->GetGroup(MExpr->GetGrpID());
    if (!explore)
    {
#ifndef IRPROP
      CONT *LocalCont = CONT::vc[ContextID];
...
void APPLY_RULE::perform()
{
  ...
  PTRACE2("APPLY_RULE performing, rule: %s expression: %s", Rule->GetName(), MExpr->Dump());
  PTRACE2("Context ID: %d , %s", ContextID, CONT::vc[ContextID]->Dump());
  PTRACE("Last flag is %d", Last);
...
CString APPLY_RULE::Dump()
{
  ...
  os.Format("APPLY_RULE rule: %s, mexpr %s, parent task %d", Rule->Dump(), MExpr->Dump(), ParentTaskNo);
